...
/**
 * 
 * During the game a player can choose between different effect of the same
 * card. This method implements the question of choosing among the possible
 * effects
 *
 */
public class AskEffect extends MessageAsk {
	 ...
	 * 
	 * @param listEffect
	 *            the effects among which the player has to choose
	 * @param prodAction
	 *            the {@link ProductionAction} performed by the player
	 * @param player
	 *            the target player
	 */
	public AskEffect(LinkedHashMap<DevelopmentCard, ArrayList<ActionEffect>> listEffect, ProductionAction prodAction,
			Player player) {
		...
		for (DevelopmentCard el : listEffect.keySet()) {
			...
			for (ActionEffect effect : listEffect.get(el)) {
				str.append(effect.toString() + "\n"); // fare i toString degli
														// effetti
			}
			str.append("\n");
		}
		str.append("Quale scegli?: "); // chiedi il nome della carta e il numero
										// dell'effetto
	...
	public void applyDefault(Model model) {
		...
		AnswerEffect ans = new AnswerEffect(this.getId(), eff);
