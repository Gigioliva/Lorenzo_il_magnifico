 ...
 * representation of the Venture Card. For this type of card, we may have
 * multiple {@link RequisiteCost}, multiple {@link EndEffect} and multiple
...
public class CardVenture extends DevelopmentCard {
	...
	/**
	 * it creates an empty Venture Card, with no {@link RequisiteCost}, no
	 * {@link EndEffect}, no {@link ImmediateEffect}
	 */
	public CardVenture() {
	 ...
	 * 
	 * @return an instance of {@link CardVenture}, with the same costs and effects as the
	 *         given one.
	 */
	@Override
	...
	/**
	 * 
	 * @return It returns an {@link ArrayList} containing all the
	 *         {@link RequisiteCost} of the card
	 */
	@Override
	 ...
	 * 
	 * @param cost
	 *            : {@link HashMap} containing a possible cost of the card.
	 * @param requisite
	 *            : {@link HashMap} containing the corresponding requisite.
	 */
	@Override
	...
	/**
	 * It calculates all the {@link Effect} that the player can afford to pay
	 * for the given card. In particular a player can afford a cost if he has in
	 * his resources all the resources required by the requisite and by the
	 * cost.
	 * 
	 * @param player
	...
	// controlla singolo requisiteCost, ritorna vero se il giocatore può
	// permetterselo
	private boolean canAffordCostRequisite(RequisiteCost item, Player player) {
		...
		for (String type : costkeys) {
			// se il giocatore non si può permettere il costo della carta,
			// ritorna false
			if (!controlSingleCost(item, type, player))
				return false;
		}
		for (String type : requisitekeys) {
			// se il giocatore non può permettersi il requisito della carta,
			// ritorna false
	...
	/**
	 * It returns the cost that the {@link Player} has to pay considering the
	 * malus and bonus that the player accumulated during the game
	 * 
	 * @param player
	 *            for which you want to get the actual costs
	 * @return an ArrayList containing the {@link RequisiteCost} updated
	 *         according to bonus and malus
	 */
	 ...
	 * @param chosenCost
	 *            the {@link RequisiteCost} to apply to the player
	 * @param player
	 ...
	 * A player can afford to take a card if exists at least one
	 * {@link RequisiteCost} that he can afford.
	 * 
	 * @param player
	...
	public String toString() {
		...
		/*
		 * if(immediateEffects.size() > 0){ str.append("immediate effects: \n");
		 * int i = 1; for(ImmediateEffect effect: immediateEffects){
		 * str.append("  " + "[" + i +"] " + effect.toString()); i++; } }
		 * 
		 * if(endEffects.size() > 0){ str.append("end effects: \n"); int i = 1;
		 * for(EndEffect effect: endEffects){ str.append("  " + "[" + i +"] " +
		 * effect.toString()); i++; } }
		 */
		return str.toString();
	}
